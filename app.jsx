...
function App() {
  ...
  // Optimized High-Performance Cosmic Running Stars & Neural Canvas
  useEffect(() => {
    ...
    if (canvas) {
      ...
      // 1. Running background stars (Optimized particle count to eliminate rAF violations)
      ...
      const render = (now) => {
        // Delta time throttling to ensure smooth 60fps without lag spikes
        if (now - lastTime < 16) {
        ...
        for (let a = 0; a < stars.length; a++) {
          ...
          // Draw star point (Zero shadowBlur for 60fps performance!)
          ctx.beginPath();
              ...
              <div className="hf-models-grid fade-in-up">
                {filteredHf.map((hf, i) => (
                  <div key={i} className="hf-card">
                    ...
                    <div className="hf-code-line"><code>{String(hf.code || '').split('\n')[0]}</code></div>
